...
def get_organization_data(org_name: str, github_token: str) -> OrganizationSchema:
    response = requests.get(
        url=f"{BASE_GITHUB_URL}/orgs/{org_name}",
        headers=_get_headers(github_token)
    )
    if response.status_code != 200:
        raise PullError(f"failed to pull '{org_name}' data for", error_code=response.status_code)
    data = response.json()
    return {
        "name": data["login"],
        "git_host": "github",
    ...
    async with aiohttp.ClientSession() as session:
        if not repo_names:
            ...
            for page in range(1, last_page + 1):
                tasks.append(asyncio.create_task(_get_repos_data(session, org_name, page, github_token)))
            ...
            for repo_name in repo_names:
                tasks.append(asyncio.create_task(_get_repo_data(session, org_name, repo_name, github_token)))
            task_results = await asyncio.gather(*tasks)
...
async def _get_repos_data(session, org_name: str, page: int, github_token: str) -> list[dict]:
...
async def _get_repo_data(session, org_name: str, repo_name: str, github_token: str) -> tuple[dict | None, int]:
    async with session.get(
